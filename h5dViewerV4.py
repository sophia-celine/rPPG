...
class FileSignalSelector:
    ...
    def process_files(self):
        for file_name in self.h5_files:
            ...
            if len(parts) >= 3:
                ...
                date = parts[1]
                hour = parts[2]
                log_file_name = f"{date}_{hour}_onLineDevices.log"
                log_file_path = os.path.join(self.directory_path, log_file_name)
                ip_to_monitor = self.read_log_file(log_file_path)
                monitor_name = ip_to_monitor.get(ip, "Unknown")
                display_name = f"{monitor_name} ({ip}) - {file_name}"
            else:
                monitor_name = "Unknown"
                display_name = f"Unknown - {file_name}"
            self.display_names.append(display_name)
            self.file_monitor_names[file_name] = monitor_name

    def read_log_file(self, log_file_path):
        if log_file_path in self.log_file_cache:
            return self.log_file_cache[log_file_path]
        ip_to_monitor = {}
        try:
            with open(log_file_path, 'r') as f:
                for line in f:
                    ...
                    if line:
                        parts = line.split(',')
                        if len(parts) == 2:
                            monitor_name, ip = parts
                            ip = ip.strip()
                            monitor_name = monitor_name.strip()
                            ip_to_monitor[ip] = monitor_name
        except Exception as e:
            print(f"Failed to read log file {log_file_path}: {e}")
        self.log_file_cache[log_file_path] = ip_to_monitor
        return ip_to_monitor
    ...
    def create_file_selection_ui(self):
        ...
        self.file_listbox = tk.Listbox(frame, selectmode=tk.SINGLE, yscrollcommand=scrollbar.set, height=30, width=80)
        scrollbar.config(command=self.file_listbox.yview)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        self.file_listbox.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # Insert display names into the listbox
    ...
    def open_selected_file(self):
        ...
        selected_file_name = self.h5_files[selected_index]
        self.current_file_path = os.path.join(self.directory_path, selected_file_name)
        self.current_monitor_name = self.file_monitor_names.get(selected_file_name, "Unknown")
        self.load_file_data()
        if self.data_loaded:
    ...
    def create_signal_selection_ui(self):
        ...
        # Update window title
        self.root.title(f"Select Signal to Plot - {os.path.basename(self.current_file_path)} - {self.current_monitor_name}")
        ...
        # Add a scrollbar to the listbox
        scrollbar = tk.Scrollbar(frame, orient=tk.VERTICAL)
        self.signal_listbox = tk.Listbox(frame, selectmode=tk.SINGLE, yscrollcommand=scrollbar.set, height=30, width=80)
    ...
    def plot_signal(self):
        ...
        plt.figure()
        plt.plot(sig)
        plt.title(f"{selected_name} - {os.path.basename(self.current_file_path)} - {self.current_monitor_name}")

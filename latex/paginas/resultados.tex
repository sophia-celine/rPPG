...
A correlação entre os sinais de rPPG e os sinais de fotopletismografia de referência dos vídeos da Coleta Preliminar 1 foi analisada pelos respectivos índices de correlação de Pearson. Os valores resultantes para os pacientes 1 e 2 são apresentados na \autoref{tab:rppg_ppg_correlation}, em que os termos entre parênteses representam as bases de dados nas quais foram treinados cada modelo. 
...
Entre os métodos não supervisionados, para todos os pacientes, o método ICA aplicado ao vídeo do paciente 3 foi o que gerou estimativas de frequência respiratória com o menor erro médio absoluto. Já entre os métodos supervisionados, o modelo PhysFormer treinado na base de dados PURE aplicado no vídeo do paciente 2 foi o que resultou no sinal cujas estimativas de frequência respiratória tiveram o menor MAE. As estimativas por janela obtidas por meio de cada um dos tipos de demodulação e por meio da fusão das demodulações desses sinais são apresentadas na \autoref{fig:rr-best}, bem como o erro por janela. Nessa figura, é possível observar, ainda, que as estimativas obtidas por meio da fusão dos três tipos de demodulação apresentam, na maioria dos pontos, um erro menor do que o erro das estimativas obtidas pelas demodulações individuais.

\begin{figure}[h!]
    \centering
\includegraphics[width=1\linewidth]{imagens/rr_best.png}
    \caption{Estimativas de frequência respiratória por janela e respectivos erros absolutos para os casos de melhor desempenho: método ICA (Paciente 3) e modelo PhysFormer treinado na base de dados PURE (Paciente 2). Os gráficos detalham os valores obtidos via demodulações de linha de base (BW), amplitude (AM) e frequência (FM), além da estimativa final resultante da fusão das três demodulações, em comparação com o sinal de referência.}
    Fonte: autoria própria
    \label{fig:rr-best}
\end{figure}
